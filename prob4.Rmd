...
From the above plots, we can see that the classes are inbalanced, with more high price laptops (1) than low price laptops (0).

The following plot shows the proportion of high price (1) and low price (0) laptops for each company in the training set. Each bar is stacked and filled by the 'high' variable: different colors for high (1) and low (0).
...
**Not significant predictors (p ≥ 0.05):** - Weight (p = 0.085)\
- Rating (p = 0.065)\
- InventoryID\
- CompanyLenovo\
- TypeName\
...
### (c) Comparison with the linear regression model
...
### (d) Interpretation of Significant Variables

| Variable | Effect on Probability of Being High-Priced | Justification |
|------------------|-----------------------------------|--------------------|
...
### (e) Comparison of coefficient signs between models
...
**Same sign in both models:** - **Memory:** Positive in both → more RAM increases the likelihood of being high-priced and also raises the price level in Euros.\
...
**Different signs between models:** - **GPUIntel:** Negative in logistic regression (though not significant) but positive in linear regression → Intel GPUs are linked with slightly lower odds of being in the high-price group, yet contribute positively to price as a continuous outcome.\
...
### (f) Predicting for a Specific Laptop
...
$$
Z = \beta_0 + \beta_1(\text{InventoryID}) + \beta_{Lenovo} + \beta_{Ultrabook} + \beta_{Intel} 
+ \beta_{Screen}(\text{Screen}) + \beta_{Memory}(\text{Memory}) + \beta_{Weight}(\text{Weight}) + \beta_{Rating}(\text{Rating})
$$
...
### (g) Model Evaluation on Test Set
